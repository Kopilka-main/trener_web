...
function ExercisesTab({ linked }: { linked: boolean }) {
...
function MeasurementsTab({ linked }: { linked: boolean }) {
  const measurements = useClientMeasurements();
  ...
  if (adding) {
    return <MeasurementForm onClose={() => setAdding(false)} />;
  }
  if (editing) {
    return <MeasurementForm measurement={editing} onClose={() => setEditing(null)} />;
  }
  ...
  return (
    <div className="flex flex-col gap-3">
      {linked ? (
        ...
        <p className="text-sm text-ink-muted">Подключите тренера, чтобы вести замеры.</p>
      )}
 ...
 */
function MeasurementsAnalytics({ items }: { items: MeasurementResponse[] }) {
...
function MeasurementForm({
  ...
  async function onSave() {
    ...
    try {
      ...
      if (measurement) {
        const args: UpdateMeasurementArgs = { mid: measurement.id, input: payload };
