 ...
 * Тап по числу или по пустому месту в колонке — открыть этот день.
 */
function WeekView({
  anchor,
  sessions,
              ...
              {dates.map((d) => {
                ...
                return (
                  ...
                  >
                    ...
                    {/* Пустая область колонки — открыть день */}
                    <button
                      type="button"
                      onClick={() => onPickDay(d)}
                      aria-label={`Открыть ${DAY_SHORT[weekdayMon(d)]} ${String(d.getDate())}`}
