...
- Два приложения на одном API: тренерское (`apps/web`) и клиентское (`apps/web-client`). Аутентификация раздельная: тренер — cookie `sid` → `req.trainerId`; клиент — cookie `client_sid` → `req.clientAccountId` (таблицы `client_accounts`/`client_sessions_auth`, модуль `client-auth`).
- Клиент НЕ выбирает scope сам: клиентские роуты получают `{trainerId, clientId}` только через резолвер `resolveScope` (по `clients.accountId` + активной связи `trainer_clients`); `repo` остаётся `trainerId`-скоупленным. Пользователь-владелец данных = тренер; клиенты — записи под управлением тренера (связь M:N через `trainer_clients`), привязка клиентского аккаунта делается тренером.
