...
/// Путь батч-отправки лога экранов тренера.
const String _analyticsEventsPath = '/api/analytics/events';

/// Путь go_router → короткий человекочитаемый ключ экрана. Префиксные маршруты
/// (карточка клиента, чат, проведение тренировки, привязка) маппим по началу.
/// Неизвестный путь оставляем как есть.
String screenKeyForPath(String path) {
  if (path == '/home') return 'main';
  if (path == '/calendar') return 'calendar';
  if (path == '/accounting') return 'finance';
  ...
  if (path.startsWith('/active/')) return 'workout';
