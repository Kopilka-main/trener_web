...
import 'package:trener_trainer/api/trainer_workouts.dart';
      ...
      'exercises': <Map<String, dynamic>>[
        <String, dynamic>{
          ...
          'sets': <Map<String, dynamic>>[
            <String, dynamic>{'setIndex': 0, 'plannedReps': 10, 'actualReps': 9, 'done': true},
          ],
        },
      ],
    };
...
void main() {
  ...
  test('успешный fetch кэширует деталь под workout_detail_<id>', () async {
    final _FakeApiClient api = _FakeApiClient((String path) async => <String, dynamic>{'workout': _rawWorkout()});
    final ProviderContainer c = buildContainer(api);

    final Workout w = await c.read(trainerWorkoutsApiProvider).fetch('cl1', 'w1');

    expect(w.name, 'Верх тела');
    expect(w.exercises, hasLength(1));

    final List<Map<String, dynamic>>? cached = await store.readList('workout_detail_w1');
    expect(cached, isNotNull);
    expect(cached, hasLength(1));
    expect(cached!.first['id'], 'w1');
  });

  test('сетевая ошибка → fetch отдаёт деталь из кэша', () async {
    // Кэш уже заполнен предыдущим онлайн-просмотром.
    await store.writeList('workout_detail_w1', <Map<String, dynamic>>[_rawWorkout(name: 'Из кэша')]);
  ...
  test('сетевая ошибка без кэша → пробрасывает исходную ошибку', () async {
    ...
    expect(
      () => c.read(trainerWorkoutsApiProvider).fetch('cl1', 'w1'),
