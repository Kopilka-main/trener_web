...
- **Авторизация:** тренеры и клиенты, email + пароль.
...
- **service** — бизнес-логика, не знает про HTTP. Принимает scope
  (`trainerId` либо `clientId`) из контекста запроса.
...
```text
clients          -- ЧЕЛОВЕК (общая идентичность):
...
```text
users        -- id, email (нормализован, уникальный индекс), password_hash,
             --   role ∈ {trainer, client}, created_at
trainers     -- id, user_id, first_name, last_name, title, specialties, bio, ...
sessions_auth-- id (session token), user_id, expires_at, created_at
```

`trainer` → ссылается на `trainers.id`; `client` → на `clients.id`.
...
- **Клиент** (один аккаунт) читает только `WHERE client_id = :me` — видит свои
  тренировки/занятия от всех своих тренеров.
- Слой `repo` обязан принимать scope — изоляция прошита в данные, а не в код
  вызова. Нарушение ловится security-тестами в CI.
...
- **Контекст:** plugin `tenant-context` на каждый запрос достаёт сессию →
  грузит user → кладёт в request `{ userId, role, trainerId?, clientId? }`.
- **Guard'ы:** `requireAuth`, `requireTrainer`, `requireClient`,
  `requireClientAccess(clientId)` (проверка связи через `trainer_clients`;
  отказ → 404, чтобы не раскрывать существование сущности).
...
```text
request → cors → rate-limit → cookie-сессия → tenant-context (userId, role, scope)
        → route (Zod-валидация) → guard → service(scope) → repo(WHERE scope)
